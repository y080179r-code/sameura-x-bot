...
class BotTests(unittest.TestCase):
    ...
    def test_high_rate_waits_24h_of_observation_time(self):
        ...
        obs = {
            "observed_at": "2026-09-05T12:00:00+09:00",
            "rate": 85.0,
        }
        prev = {
            "observed_at": "2026-09-05T11:00:00+09:00",
            "rate": 85.0,
        }

        state = bot.default_state()
        state["last_posted_at"] = datetime(
            2026, 9, 5, 10, 0, tzinfo=JST
        ).isoformat()
        ...
        state["last_posted_observed_at"] = "2026-09-04T12:00:00+09:00"
    ...
    def test_under_10_uses_observation_time_not_post_time(self):
        ...
        state["last_posted_at"] = datetime(
            2026, 9, 5, 11, 55, tzinfo=JST
        ).isoformat()

        self.assertFalse(
            bot.choose_decision(obs, state, prev, True, now).post
        )

        now2 = datetime(2026, 9, 5, 12, 37, tzinfo=JST)

        d = bot.choose_decision(
            obs,
            state,
            obs,
            False,
    ...
    def test_under_10_posts_next_hour_even_if_previous_post_was_late(self):
        ...
        prev = {
            "observed_at": "2026-09-05T18:00:00+09:00",
            "rate": 8.8,
        }
    ...
    def test_post_has_no_url_and_rainfall_zero_is_shown(self):
        obs = {
        ...
        text = bot.build_post(
            ...
            state,
            prev,
            bot.Decision(True, "test", "regular"),
        )
    ...
    def test_extract_river_rows(self):
        ...
        self.assertEqual(rows[-1]["rainfall_mm_h"], 1.2)
        self.assertEqual(
            rows[-1]["observed_at"],
...
class TestMoodV4(unittest.TestCase):
    ...
    def test_movement_comment(self):
        self.assertEqual(
            bot.movement_comment(1.2, 30),
            "いい感じに増えてます😊",
        )

        self.assertEqual(
            bot.movement_comment(-2.2, 30),
            "大きく減少しています🚨",
        )
...
class TestDroughtStatusV45(unittest.TestCase):
    ...
    def test_historical_restriction_alone_is_not_current(self):
        ...
        self.assertIsNone(
            bot.parse_drought_status_text(text)
        )
    ...
    def test_drought_hashtag_only_when_official_restriction_active(self):
        ...
        state["history"] = [
            prev,
            {
                "observed_at": base_obs["observed_at"],
                "rate": base_obs["rate"],
            },
        ]
        ...
        normal_obs = dict(
            base_obs,
            drought_restriction_active=False,
        )
...
class TestStorageTrendV46(unittest.TestCase):
    def test_storage_trend_emoji(self):
        self.assertEqual(
            bot.storage_trend_emoji(28120.0, 28110.0),
            "🔺",
        )

        self.assertEqual(
            bot.storage_trend_emoji(28100.0, 28110.0),
            "🔽",
        )

        self.assertEqual(
            bot.storage_trend_emoji(28110.0, 28110.0),
            "➖",
        )

        self.assertEqual(
            bot.storage_trend_emoji(28110.0, None),
            "",
        )
...
class TestRateChangeArrowV49(unittest.TestCase):
    def test_rate_change_arrows(self):
        self.assertEqual(
            bot.change_emoji(0.1),
            "↗️",
        )

        self.assertEqual(
            bot.change_emoji(0.0),
            "➡️",
        )
        ...
        self.assertEqual(
            bot.change_emoji(None),
            "",
        )

    def test_flat_change_still_shows_zero_and_right_arrow(self):
        obs = {
            "observed_at": "2026-09-11T08:00:00+09:00",
            "rate": 8.1,
            "rainfall_mm_h": 0.0,
            "storage_thousand_m3": 29190.0,
            "inflow_m3_s": 7.4,
            "outflow_m3_s": 0.0,
            "source": "国土交通省 川の防災情報",
            "source_url": bot.RIVER_URL,
        ...
        prev = {
            "observed_at": "2026-09-11T07:00:00+09:00",
            "rate": 8.1,
            "storage_thousand_m3": 29100.0,
        }
